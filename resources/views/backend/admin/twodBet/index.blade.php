<div>
    <x-admin.pages-header title="2D ထိုးစားရင်း" :breadcrumbs="[['label' => '2D ထိုးစားရင်း', 'url' => route('admin.payment-record')], ['label' => 'List']]" :permission="Gate::check('deposit_request_create_false')" :route="route('admin.deposit-request', ['action' => 'create'])" />
    ...
    <div class="rounded-lg shadow-lg bg-primary-500 ">
        ...
        <x-admin.table>
            ...
            <x-slot name="header">
                <tr class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-900 dark:text-gray-400">
                    ...
                    <th scope="col" class="px-6 py-3">two D နံပါတ်</th>
            ...
            <!-- Body Slot -->
            <x-slot name="body">
                @forelse($twodBets as $index => $record)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            <span
                                class="text-black dark:text-white">{{ config('constant.session_time.' . $record->twodledger->session_time, '-') }}</span>
                        </td>
                        <td class="px-6 py-4">
        ...
        <x-admin.twodwin-modal title="လျော်မှာသေချာပါသလား?" />
    </div>

    <!-- Pagination -->
    <div class="p-4 bg-gray-100 dark:bg-gray-800 rounded-b-xl">
        {{-- {{ $users->links() }} --}}
        {{ $twodBets->links(data: ['scrollTo' => true]) }}
