<div>
    <x-admin.pages-header title="Total Balances" :breadcrumbs="[['label' => 'Total Balances', 'url' => route('admin.total-balance')], ['label' => 'List']]" :permission="Gate::check('total_balance_create_false')" :route="route('admin.total-balance', ['action' => 'create'])" />
        ...
        <x-admin.table>
            ...
            <x-slot name="header">
                <tr class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-900 dark:text-gray-400">
                    ...
                    <th scope="col" class="px-6 py-3">Total Balance</th>
                            ...
                            <x-admin.action-dropdown>
                                @can('total_balance_show')
                                    <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <a href="{{ route('admin.total-balance', ['action' => 'show', 'id' => $user->id]) }}"
                                ...
                                {{-- @can('customer_edit')
                                    <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <button href="{{ route('admin.customer', ['action' => 'edit', 'id' => $user->id]) }}"
                                            class="flex items-center gap-2 px-4 py-2" wire:navigate>
