<div>
    <x-admin.pages-header title="ငွေသွင်းစာရင်း" :breadcrumbs="[['label' => 'Deposit Request', 'url' => route('admin.deposit-request')], ['label' => 'List']]" :permission="Gate::check('deposit_request_create_false')" :route="route('admin.deposit-request', ['action' => 'create'])" />
    ...
    <div class="rounded-lg shadow-lg bg-primary-500">
        ...
        <div class="flex flex-wrap items-center justify-between p-4 bg-gray-100 dark:bg-gray-800 rounded-t-xl">
            ...
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.away="open = false"
                    class="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-700 shadow-lg rounded-lg p-4 z-10">
                    <div class="mb-3">
                        <label class="block text-sm text-gray-700 dark:text-gray-300">Status:</label>
                        <div class="relative">
                            <x-admin.select wire:model="statusFilter" wire:change="filterStatus">
                                <option value="">All</option>
                                @foreach (config('constant.paid_status') as $key => $value)
        ...
        <x-admin.table>
            <x-slot name="header">
                <tr class="text-xs text-gray-700 uppercase bg-gray-200 dark:bg-gray-900 dark:text-gray-400">
                    ...
                    <th scope="col" class="px-6 py-3">လုပ်ဆောင်မှုအမှတ်</th>
                    <th scope="col" class="px-6 py-3">အကောင့်အမည်</th>
                    <th scope="col" class="px-6 py-3">အကောင့်နံပါတ်</th>
                    <th scope="col" class="px-6 py-3">ငွေလွဲဝင်သော အကောင့်</th>
            ...
            <x-slot name="body">
                @forelse($depositRequests as $index => $deposit)
                    <tr
                        ...
                        <td class="px-6 py-4">
                            <button type="button"
                                class="inline-flex items-center justify-center px-4 py-2 rounded-md text-white
                                    {{ $deposit->status == 2 ? 'bg-green-600 cursor-not-allowed' : ($deposit->status == 3 ? 'bg-red-600 cursor-not-allowed' : 'bg-yellow-500 hover:bg-yellow-400') }}"
                                {{ in_array($deposit->status, [2, 3]) ? 'disabled' : '' }}
                        ...
                        <td class="px-6 py-4">
                            {{ $deposit->created_at ? $deposit->created_at->format('d M, Y') : '-' }}
                        </td>
                        <td class="px-6 py-4 relative">
                            <x-admin.action-dropdown>
                                @can('deposit_request_show')
                                    <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <a href="{{ route('admin.deposit-request', ['action' => 'show', 'id' => $deposit->id]) }}"
                                ...
                                @can('deposit_request_edit_false')
                                    <li class="hover:bg-gray-100 dark:hover:bg-gray-600">
                                        <a href="{{ route('admin.deposit-request', ['action' => 'edit', 'id' => $deposit->id]) }}"
                ...
                @empty
                    <tr>
                        <td colspan="10" class="px-25 py-4 text-center text-gray-500 dark:text-gray-400">
                         ယနေ့အတွက် ငွေသွင်းစာရင်းများမရှိပါ.
                        </td>
                    </tr>
                @endforelse
            </x-slot>
        ...
        <!-- Paid Modal -->
        <x-admin.modal.alert-model title="Paid လုပ်မှာသေချာပါသလား?" :amount="$selectedAmount" :working_number="$selectedWorkingNumber"
            :account_name="$selectedAccountName" :account_number="$selectedAccountNumber" />
    </div>

    <!-- Pagination -->
    <div class="p-4 bg-gray-100 dark:bg-gray-800 rounded-b-xl">
        {{ $depositRequests->links(data: ['scrollTo' => true]) }}
    </div>
</div>
